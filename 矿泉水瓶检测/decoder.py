...
def get_decoder(pred):
    ...
    pred = pred.data
    pred = pred.squeeze(0)  # 将维，四维变成三维[14,14,24]
    contain1 = pred[:, :, 4].unsqueeze(2)
    # print('contain1{}'.format(contain1))
    contain2 = pred[:, :, 9].unsqueeze(2)
    # print('contain2{}'.format(contain2))
    contain = torch.cat((contain1, contain2), 2)
    mask1 = contain > 0.7  # 大于阈值
    mask2 = (contain == contain.max())  # we always select the best contain_prob what ever it>0.9pred[:,:,4]
    mask = (mask1 + mask2).gt(0)
    # print('mask1{}mask2{}mask{}'.format(mask1,mask2,mask))
        ...
        for j in range(grid_num):
            for b in range(2): # 两个预测框
                if mask[i, j, b] == 1: #有目标物体的
                    ...
                    xy = torch.FloatTensor([j, i])   # cell左上角  up left of cell  [14,14]
                    # print('xy{}'.format(xy))
                    xy = xy * cell_size
                    # print('xy{}'.format(xy))
                    ...
                    box_xy[2:] = box[:2] + 0.5 * box[2:]
                    max_prob, cls_index = torch.max(pred[i, j, 10:], 0)  # 取出14个分类中最大的索引即为当前分类,0为按列
