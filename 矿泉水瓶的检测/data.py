...
def get_bbox(label_list,image_list):
    ...
    for xml_file,image_file in zip( label_list,image_list):  # 同时拿出一张图片的路径和对应label路径（图片路径和label路径是生成时对应好的）
        ...
        for obj in tree.findall('object'):
            ...
            obj_struct = [int(bbox.find('xmin').text),     # 列表形式存放一个bbox坐标信息
                                  int(bbox.find('ymin').text),
                                  int(bbox.find('xmax').text),
                                  int(bbox.find('ymax').text)]
            object.append(obj_struct)
        bboxes.append(object)
        # draw_img_rects(cv_img, obj_struct)
...
# 在图片中画出bbox
def draw_img_rects(image, obj_struct):
    xmin, ymin, xmax, ymax = obj_struct['bbox']
    w = xmax - xmin
    h = ymax - ymin
    cv2.rectangle(image, (int(xmin), int(ymin)), (int(xmin) + int(w), int(ymin) + int(h)), (0, 255, 0), 2) # 在图片中画矩形框
...
if __name__ == '__main__':
    ...
    i,j = get_mean_std(image_path)

    '''step 05 编写 class myDataset(DataLoader)'''
    train_dataset = myDataset(image_path=image_path, labels_list=label_list, bboxes=bboxes,train=True, transform=[transforms.ToTensor()])
